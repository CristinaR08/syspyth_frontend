...
export default {
    ...
    methods: {
        async aplicarFiltros() {
            try {
                const response = await axios.get('http://localhost:8080/api/v1.0/administrador/lista');
        ...
        async fetchAdminCedula() {
            if (this.cedula) {
                try {
                    const response = await axios.get(`http://localhost:8080/api/v1.0/administrador/consultar/${this.cedula}`);
                    this.admins = [response.data];
                } catch (error) {
