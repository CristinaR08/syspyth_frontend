...
export default {
  ...
  methods: {
    async aplicarFiltros() {
      try {
        const response = await axios.get('http://localhost:8080/api/v1.0/estudiantes/lista');
    ...
    async buscar() {
      try {
        let response;
        if (this.cedula) {
          response = await axios.get(`http://localhost:8080/api/v1.0/estudiantes/consultar/${this.cedula}`);
          this.students = [response.data];
        } else {
